...
func (spider Spider) GetBishijie() []map[string]interface{} {
	...
	url := "https://iapi.bishijie.com/news/list?page=1&size=20&signature=c0f18c5477b45ddce5a619e95318f566&ts=1575599744"
	timeout := time.Duration(5 * time.Second) //超时时间5s
	client := &http.Client{
		Timeout: timeout,
	}
	...
	request.Header.Add("uuid", `F774590C12C247800E9E9BF42EEDFF26`)
	res, err := client.Do(request)
	if err != nil {
		fmt.Println("抓取theblockbeats" + spider.DataType + "失败")
		return []map[string]interface{}{}
